...
def ocr_paddle_lines(img: Image.Image, paddle_ocr, enhance=True):
    import numpy as np
...
def run_ocr(img, engine, lang, pytesseract=None, paddle_ocr=None, enhance=True):
    if engine == "tesseract":
        return ocr_tesseract_lines(img, lang, pytesseract)
    elif engine == "paddle":
        return ocr_paddle_lines(img, paddle_ocr, enhance=enhance)
    else:
...
def main():
    ...
    parser.add_argument("--max-docs", type=int, default=None,
                        help="Limite le nombre de fichiers traités (ex: 10 pour un petit échantillon)")
    parser.add_argument("--no-enhance", action="store_true",
                        help="Désactive le filtre de contraste (pour comparer avec/sans)")
    ...
    if args.max_docs is not None:
        ...
        print(f"Échantillon limité à {len(pairs)} fichier(s) (--max-docs {args.max_docs})")
